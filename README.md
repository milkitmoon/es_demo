...
> 사용자 정보를 등록과 검색 기능의 분리
- 사용자 정보 등록/수정 : UserHandlerServiceImpl 를 통해 신규/변경 사용자정보는 h2db 및 elasticsearch 에 같이 반영됩니다
- 사용자 정보 검색 : 단일건 검색이 아닌 조건 검색의 경우 UserHandlerServiceImpl 에서 elasticsearch 를 통해 문서를 검색하게 됩니다 (여기에서 효율을 높이기 위해서는 전체문서를 가져오기 보다는 문서의 ids 만을 추출하여 h2db 에서 검색하도록 하는게 좋을거 같다)
